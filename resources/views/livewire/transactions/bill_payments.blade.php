...
new class extends Component {
    ...
    private function applyFilters($query)
    {
        ...
        // Reason types (filtre sur reason_type = index)
        if (!empty($this->Ref_invoice)) {
            $query->whereIn('reason_index', $this->Ref_invoice);
        }
                ...
                <table style="width:100%; border-collapse:collapse; font-size:12px;">
                    <thead>
                        <tr style="background:#F9FAF9;">
                            ...
                            <th style="padding:10px 14px; text-align:left; color:#6b7280; font-weight:500; border-bottom:1px solid #e5e7eb; white-space:nowrap;">Invoice Number</th>
                            ...
                            <th style="padding:10px 14px; text-align:left; color:#6b7280; font-weight:500; border-bottom:1px solid #e5e7eb; white-space:nowrap;">Balance Before</th>
                            <th style="padding:10px 14px; text-align:left; color:#6b7280; font-weight:500; border-bottom:1px solid #e5e7eb; white-space:nowrap;">Balance After</th>
                    ...
                    <tbody>
                        @forelse($transactions as $transaction)
                            <tr style="border-bottom:1px solid #f3f4f6;"
                                ...
                                <td style="padding:10px 14px; color:#374151;">{{number_format($transaction->balance_before_credit * 100, 0, ',', ' ') }} </td>
                                <td style="padding:10px 14px; color:#374151;">{{number_format($transaction->balance_after_credit * 100, 0, ',', ' ') }}</td>
